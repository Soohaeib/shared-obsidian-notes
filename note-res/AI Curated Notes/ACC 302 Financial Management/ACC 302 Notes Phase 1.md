...
```mermaid
flowchart TD
    A[Stockholders / Owners] -->|Elect| B[Board of Directors]
    B -->|Hire & Oversee| C[President / CEO]
    C -->|Directs| D[CFO & COO]
    
    classDef highlight stroke:#00a3cc,stroke-width:2px;
...
```mermaid
flowchart TD
    BOD[Board of Directors] --> CEO[President / CEO]
    CEO --> CFO[Chief Financial Officer]
    
    CFO --> TREAS[Vice President / Treasurer]
    CFO --> CONT[Vice President / Controller]
    
    TREAS --> T1(Capital Budgeting)
    TREAS --> T2(Cash & Working Capital)
    TREAS --> T3(Banking Relations)
    TREAS --> T4(Credit & Dividend Mgt)
    
    CONT --> C1(Financial & Cost Accounting)
    CONT --> C2(Tax Analysis & SEC Reporting)
    CONT --> C3(Operating Budgets)
    CONT --> C4(Internal Auditing)
    
    classDef core stroke:#ffb347,stroke-width:2px;
    classDef functional stroke:#4da6ff,stroke-width:2px;
    class BOD,CEO,CFO core;
    class TREAS,CONT functional;
```
...
```mermaid
flowchart LR
    P0((Period 0)) -- "Multiply by (1 + r)^n" --> Pn((Period n))
    P0 -.- V0[PV_0]
    Pn -.- Vn[FV_n]
...
```mermaid
flowchart RL
    Pn((Period n)) -- "Divide by (1 + r)^n" --> P0((Period 0))
    Pn -.- Vn[FV_n]
    P0 -.- V0[PV_0]

    classDef node stroke:#ffb347,stroke-width:2px;
    class P0,Pn node;
```
